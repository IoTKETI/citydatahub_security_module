          ...
          DB.ByQuery("SELECT * FROM USERS INNER JOIN VERICODE on (USERS.USER_ID_PK=VERICODE.VERICODE_FK1) WHERE USERS.USER_ID_PK="+"\'"+userId+"\' AND USERS.EMAIL=\'"+userEmail+"\'",(err,result,resCount)=>{
            if(err){
              ...
              let mailOptions = {
                ...
                to : userEmail,
                subject : '안녕하세요. 스마트시티 도시데이터허브 비밀번호 찾기 인증 메일입니다.',
                html : "<h1>Hello SmartCity!</h1></p>"+"<h3>안녕하세요. 스마트시티 도시데이터허브 서비스 비밀번호 찾기 인증용 메일입니다. 15분 내로 인증해주세요.</h3></p>"+"<h3> 아래의 이미지를 클릭하여 비밀번호 찾기를 진행해주세요. 본인이 아닌 경우 메일을 삭제하시기 바랍니다.</h3>"+
                        "<a href=\'http://"+serverInfo.serverip+serverInfo.serverport+"/accounts/resetPwd?email="+userEmail+"&verifyCode="+verifyCode+"\'>이 글을 클릭하여 비밀번호를 변경해 주세요.</a>"
              };
